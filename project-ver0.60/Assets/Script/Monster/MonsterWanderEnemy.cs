...
public class MonsterWanderEnemy : MonoBehaviour
{
    ...
    public float alertRadius = 10;          //警戒半径，玩家进入后怪物会发出警告，并一直面朝玩家
    public float defendRadius = 5;          //自卫半径，玩家进入后怪物会追击玩家，当距离<攻击距离则会发动攻击（或者触发战斗）
    public float chaseRadius = 25;          //追击半径，当怪物超出追击半径后会放弃追击，返回追击起始位置
    ...
    private float lastActTime = 0;             //最近一次指令时间
    ...
    void Attack(string atk)//攻擊模式
    {
        if (atk == "1")
        {
            ...
            if (number <= AttackWeight[0])
            {
                ...
                thisAnimator.SetBool("SK1", false);
                thisAnimator.SetBool("SK2", false);
                thisAnimator.SetBool("SK3", false);
            }
            if (AttackWeight[0] < number && number <= AttackWeight[0] + AttackWeight[1])
            {
                thisAnimator.SetBool("ATK", false);
                thisAnimator.SetBool("SK1", true);
                thisAnimator.SetBool("SK2", false);
                thisAnimator.SetBool("SK3", false);
            }
